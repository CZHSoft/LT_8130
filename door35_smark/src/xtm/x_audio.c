...
#define MIC_VOLUME  0x60 //0x88
#define MIC_VOLUME_DEC  0x40 //0x88
INT8U RecAudioCount = 0;
...
#define _HALF_DUPLEX    //半双工
 ...
 #define  MIC_DB_DEC  0x24 //0x1E//0xc
...
void StartNoiseDect(void)
{
  ...
  #ifdef _HALF_DUPLEX    //半双工
   #ifdef _NOISE_DECT   //噪声检测
    WriteCorrect_es8388(0x19, 0x22);   //固定0x20 可能要调整   e0    soft ramp
    SetMicVolume();
...
#define SEND_RECORD_MAX_VALUE  0x2000 //0x2000//0x80//0x80//0x120 //0x80 //0x20*4
...
void Cal_PcmTotalNum(INT8U pcmi_h, INT8U pcmi_l, INT8U pcmo_h, INT8U pcmo_l)
{
  ...
  PlayPcmTotalNum[0] = (PlayPcmTotalNum[0]<<8) + pcmo_l;
    ...
    NoisePcmTotalNum[NoiseNo] = RecPcmTotalNum[0];
   ...
   //   if((DealCount % 500) == 0)
   //     xtm_printf("Cal_PcmTotalNum:: pcmi_l = 0x%X, pcmo_h = 0x%X\n", pcmi_l, pcmo_h);
}
//---------------------------------------------------------------------------
#ifdef _NOISE_DECT   //噪声检测
INT8U Noise_Count = 0;
