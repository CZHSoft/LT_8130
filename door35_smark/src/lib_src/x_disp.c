		...
		if(*ch>0x80){
	  	...
	  	addr=addr2+32;
    		...
    		addr1+=0x400000;
                        ...
                        xtm_read25x32data(addr1, dtemp, 48);//36);
	  		addr2=addr;
				for(i=0;i<24;i++){
					outportb(FONT_WR_MOST_ADD, (addr>>16));
					outportb(FONT_WR_HIGH_ADD, (addr>>8));
		  		...
		  		outportb(FONT_WR_DATA, dtemp[i*2]);
		  		outportb(FONT_WR_DATA, dtemp[i*2+1]);
                                    //    outportb(FONT_WR_DATA, hzk_data[i*2]);
                                    //    outportb(FONT_WR_DATA, hzk_data[i*2+1]);
                             /*   if((i%2) == 0)
                                 {
                                  outportb(FONT_WR_DATA, dtemp[i*3/2]);
                                  tmp = dtemp[i*3/2+1];
                                  tmp = tmp & 0xf0;
                                  outportb(FONT_WR_DATA, tmp);
                                 }
                                else
                                 {
                                  tmp = dtemp[i*3/2];
...
INT8U DisplayImage(struct TImage *t_image, INT8U FromType, INT8U FullPageType)
{
 ...
 /* xtm_printf("读 Faddr = 0x%X%X , Filelen = 0x%X%X, Flen = 0x%X%X, Width = %d, Height = %d, xLeft = %d, yTop = %d\n", \
   ...
   Set_Decode_Clock(1); //操作视频解码时钟  1  开   0  关
        ...
        if(FromType == FROMFILE)
         {
          ...
          while(1)
           ...
           #else
            if((disp_jpeg_bitstream[t_image->Len - 2] == 0xff)&&(disp_jpeg_bitstream[t_image->Len - 1] == 0xd9))
           #endif
             {
              if(ReadNum > 1)
    ...
    if(DecodeNum > 1)
     {
      xtm_printf("DisplayImage::DecodeNum = %d\n", DecodeNum);
     // ErrorStopDecode = 1;
     } 
        if(!FullPageType)
        ...
        if(FromType == FROMFILE)
         {
          ...
          while(1)
           ...
           #else
            if((disp_jpeg_bitstream[t_button->Len[buttontype] - 2] == 0xff)&&(disp_jpeg_bitstream[t_button->Len[buttontype] - 1] == 0xd9))
           #endif
             {
              if(ReadNum > 1)
