    ...
    //  printf("Local.Status = %d\n", Local.Status);
      if((Local.Status == 2)||(Local.Status == 6)
        ...
        ||(Local.Status == 3) || (Local.Status == 89))
       {
        ...
        //命令
        if((Local.Status == 2)||(Local.Status == 6)||(Local.Status == 8)||(Local.Status == 10)||(Local.Status == 89))  //对讲
          sendb[6] = VIDEOTALK;
        ...
        if((Local.Status == 2)||(Local.Status == 4)||(Local.Status == 6)||(Local.Status == 8)||(Local.Status == 10)||(Local.Status == 89))  //对讲时，本机为被叫方
         {
    ...
    switch(Local.Status)
     {
      ...
      case 89:
             ...
             {
               Local.TalkTimeOut = ADTIMEOUT; //通话最长时间
               TalkEnd_Func();
               if(Local.Status == 0)
                 Local.OnlineFlag = 0;
               Local.TimeOut = 0;
...
//---------------------------------------------------------------------------
void AdCtrlFunc(void)  //对讲控制，显示通话时间和判断超时
{
  int ATimeOut;
  if((Local.TimeOut % TIMERPERSEC)==0)
  {
        ...
        {
               Local.StreamTimeOut= ADTIMEOUT; //通话最长时间
  ...
  Local.TimeOut ++;       //监视超时,  通话超时,  呼叫超时，无人接听,ad
}
//---------------------------------------------------------------------------
//检测网络信号
void CheckEth(void)
